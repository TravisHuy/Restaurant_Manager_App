...
class OrderItemRepository @Inject constructor(private val orderItemService: OrderItemService) {
    ...
    suspend fun getOrderItemById(orderItemId:String) : Flow<Resource<OrderItem>> = flow {
    ...
    suspend fun getListOrderItem(orderItemIds: List<String>) : Flow<Resource<List<OrderItem>>> = flow {
        emit(Resource.Loading())
        try {
